...
LayoutBlock FindLayoutForSymbol(const std::vector<LayoutBlock>& blocks, char symbol)
{
...
std::vector<BreakoutGameLevel> BreakoutGameLevel::LoadLevelsFromFile(const std::string & filePath)
{
	...
	layoutCommand.parseFunc = [&](ParseFuncParams params) {
		...
		for (int c = 0; c < params.line.length(); ++c)
		{
			if (params.line[c] != '-')
			{
				LayoutBlock layoutBlock = FindLayoutForSymbol(layoutBlocks, params.line[c]);
				Block b;
